...
class LoadAction(PAB):
    ...
    def __call__(self, fpaths: list[str], ftype: str=None) -> list[TimeData]:
        ...
        loads TDMS data using `load_tdms` from `.data_loader`, and extends
        the result list with TimeData objects.
...
class TruncAction(ActionBase):
    ...
    def __call__(self, channels: list[TimeData], duration: tuple[float, float]=[0, 0]) -> list[TimeData]:
        ...
        duration : tuple[float, float], optional
            A tuple (start_time, end_time) in seconds.
...
class ResampleAction(ActionBase):
    ...
    def __call__(self, channels: list[TimeData], dt: float=1) -> list[TimeData]:
        ...
        dt : float, optional
            The desired new time interval in seconds, by default 1.
...
class EnvelopeTimeAction(PAB):
    ...
    def __call__(self, channels: list[TimeData], restore_offset: bool=False) -> list[TimeData]:
        ...
        restore_offset : bool, optional
            If True, the original offset is added back to the
            envelope, by default False.
...
class CounterToTachoAction(ActionBase):
    ...
    def __call__(self, channel: TimeData, ppr: int=1024, sr_delta: float=0.1) -> TimeData:
        ...
        ppr : int, optional
            Pulses per revolution of the encoder, by default 1024.
        sr_delta : float, optional
            Time window (in seconds) over which to calculate the RPM.
            Effectively `2 * sr_delta` is the differentiation window.
            Defaults to 0.1.
...
class RefPulseToAzimuthAction(PAB):
    ...
    # Add a docstring for the __call__ method if it were implemented
    def __call__(self):
        """(Placeholder) Creates an azimuth signal using a reference pulse.

        .. note::
            The actual implementation is missing, currently a `...` statement.

        Presumably, this action would take a reference pulse (e.g., once per
        revolution) and generate a continuous azimuth signal, possibly
        interpolating or using another speed reference.
        """
        ...
